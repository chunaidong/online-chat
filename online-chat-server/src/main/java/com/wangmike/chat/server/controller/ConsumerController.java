...
/*  39 */.getAllErrors().get(0)).getDefaultMessage());
        ...
        /*  41 */
        return this.consumerService.registry(consumerRequest, request);
    }
...
/*  50 */.getAllErrors().get(0)).getDefaultMessage());
        ...
        /*  52 */
        return this.providerService.getProviderList(providerListRequest, request);
    }
    ...
    public CommonResponse getProviderDetail(String providerId, HttpServletRequest request) {
        ...
        if (StringUtils.isBlank(providerId)) {
            /*  62 */
            return ResponseUtils.buildErrorResponse(ErrorCodeEnum.PARAM_IS_BLANK.getCode(), "主键不能为空");
        }
        /*  64 */
        return this.providerService.getProviderDetail(providerId, request);
    }
    ...
    public CommonResponse markProvider(@RequestBody String providerId, HttpServletRequest request) {
        ...
        /*  76 */
        return this.providerService.markProvider(providerId, request);
    }


    @GetMapping({"/findConsumerBalance"})
    @ApiOperation("获取当前消费者余额")
    /*  83 */ public CommonResponse findConsumerBalance(HttpServletRequest request) {
        return this.consumerService.findConsumerBalance(request);
    }
    ...
    public CommonResponse unmarkProvider(@RequestBody String providerId, HttpServletRequest request) {
        ...
        /*  95 */
        return this.providerService.unmarkProvider(providerId, request);
    }
...
/* 104 */.getAllErrors().get(0)).getDefaultMessage());
        ...
        /* 107 */
        return this.consumerService.orderProvider(consumerOrderRequest, request);
    }
...
/* 116 */.getAllErrors().get(0)).getDefaultMessage());
        ...
        /* 118 */
        return this.consumerService.getAllOrderList(consumerOrderListRequest, request);
    }
...
/* 126 */.getAllErrors().get(0)).getDefaultMessage());
        ...
        /* 128 */
        return this.consumerService.getAllMarkedList(markedRequest, request);
    }


    @GetMapping({"/getAllFlowerList"})
    @ApiOperation("获取所有鲜花列表")
    /* 134 */ public CommonResponse getAllFlowerList(HttpServletRequest request) {
        return this.consumerService.getAllFlowerList(request);
    }


    @PostMapping("/updateStateToWaitConfirm/{orderId}")
    @ApiOperation("订单由待沟通改为待确认")
    @ApiImplicitParams({@ApiImplicitParam(name = "orderId", value = "订单id", dataType = "String", required = true)})
    /* 142 */ public CommonResponse updateStateToWaitConfirm(HttpServletRequest request, @PathVariable("orderId") String orderId) {
        return this.consumerService.updateStateToWaitConfirm(request, orderId);
    }


    @PostMapping("/updateStateToChatting/{orderId}")
    @ApiOperation("订单由待确认改为沟通中")
    @ApiImplicitParams({@ApiImplicitParam(name = "orderId", value = "订单id", dataType = "String", required = true)})
    /* 151 */ public CommonResponse updateStateToChatting(HttpServletRequest request, @PathVariable("orderId") String orderId) {
        return this.consumerService.updateStateToChatting(request, orderId);
    }

    @GetMapping("/getConsumerData")
    @ApiOperation("获取当前消费者生日和等级信息")
    public CommonResponse getConsumerData(HttpServletRequest request){
        return consumerService.getConsumerData(request);
    }
}
