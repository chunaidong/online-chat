...
public class TypeCollocationServiceImpl
    ...
    @Transactional
    /* 27 */ public void insertList(List<TypeCollection> collectionList) {
        collectionList.stream().forEach(typeCollection -> this.typeCollectionMapper.insert(typeCollection));
    }


    /* 38 */
    public List<ProviderType> getProviderTypes(String providerId) {
        return this.typeCollectionMapper.getProviderTypes(providerId);
    }


    @Transactional
    /* 43 */ public void deleteTypes(String openId) {
        this.typeCollectionMapper.deleteTypes(openId);
    }
}
